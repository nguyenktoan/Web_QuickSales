...
const SearchPage = () => {
  ...
  const [encodedSearchValue, setEncodedSearchValue] = useState("");
  const searchParams = useSearchParams();
  const router = useRouter(); // Sử dụng useRouter từ thư viện next/navigation
  const page =
  ...
  const fetchData = async (keyword) => {
    try {
      ...
      const encodedSearchValue = encodeURIComponent(keyword);
      const res = await callAPI(
        `/products?populate=*&filters[$and][0][name][$contains]=${encodedSearchValue}&pagination[pageSize]=${pageSize}&pagination[page]=${page}`,
        "GET"
      );
      ...
      setLoading(false); // Kết thúc loading khi fetch dữ liệu xong (thành công hoặc thất bại)
    }
  };
  const prev = () => {
  ...
  return (
    <div>
      <div className="mt-5 ml-10 text-left text-gray-900 ">
        <span className="font-bold "> Kết quả liên quan đến:</span>
        <span className="mt-5 ml-1 text-left text-blue-600 font ">
          {encodedSearchValue}{" "}
        </span>
      </div>
      <section
